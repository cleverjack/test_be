 ...
 * Time: 6:23 PM
 */
namespace App\Classes\admin\Bus;
use App\Models\admin\BusData;
...
class Bus
{
    ...
    public static function saveBus($data)
    {
        try {
            ...
            if (!empty($data['_id'])) {
                BusData::where('_id', $data['_id'])->update($data);
                ...
                DB::collection('bus_data')->insert($userProperties);
                $userProperties['id'] = $userProperties['_id']->{'$id'};
                unset($userProperties['_id']);
                return Api::success(2030, $userProperties, ['Bus']);
            }
        } catch (\Exception $e) {
            die(exception($e));
        }
     ...
     * @return mixed
     */
    public static function getAllBus($wheres = array(), $return = array(), $skip = 0, $limit = 0)
    {
        try {
            $db = DB::collection('bus_data');
            if ($limit == 0) {
            ...
            return Api::success(6010, $data, array('Bus fetched'));
        } catch (\Exception $e) {
            die(exception($e));
        }
    }
     ...
     * @return array|string
     */
    public static function getBusInfo($wheres = array(array()), $returnColumns = array())
    {
        try {
            if (!empty($wheres)) {
                foreach ($wheres as $where) {
                    ...
                    $db = DB::collection('bus_data')->where($where[0], $where[1], $where[2]);
            ...
            return Api::success(6010, $data, array('Bus info fetched'));
        } catch (\Exception $e) {
            die(exception($e));
        }
    }
     ...
     * @return array|string
     */
    public static function deleteBus($wheres = array(array())){
        try {
            if (!empty($wheres)) {
                foreach ($wheres as $where) {
                    if (count($where) <= 1 || count($where) >= 4) {
            ...
            return Api::success(6010, $data, array('Bus deleted successfully'));
        } catch (\Exception $e) {
            die(exception($e));
        }
    }
