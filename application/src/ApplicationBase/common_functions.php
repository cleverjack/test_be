...
use App\Classes\admin\Auth\Auth;
...
function verifySession($token)
{
    try {
         ...
         */
        if(empty($token)){
            return Api::error(100, $token, array("The token can't be empty!"));
        }

        try {
            $_id = new \MongoId($token);
        } catch (\Exception $e) {
            return Api::error(100, array($token), array("Invalid Token"));
        }
        ...
        $sessionInfo = Auth::getSessionToken($conditions);
        ...
        $output = array(
            'token' => $sessionInfo['id'],
            'id' => $sessionInfo['user_id'],
        );

        return Api::success(2070, $output, array('Token'));
    } catch (\Exception $e) {
        die(exception($e));
    }
}
...
function validateInput($defaultInputs = array(), $rules = array(), $onlyJson = false)
{
    ...
    if ($validator->fails()) {
        return \ApplicationBase\Facades\Api::error(1000, array('inputs' => $inputs), array($validator->messages()->first()));
    }
...
function exception($e)
{
    ...
    if (!is_a($e, 'Exception')) {
        return \ApplicationBase\Facades\Api::error(100, array('error' => 'Exception'), array($msg), true, array($msg));
    }
 ...
 * @return array
 */
function getSingleLevelArray(array $arrData, array $path = [])
{
    $arrResult = [];
